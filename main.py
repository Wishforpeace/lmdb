...
class LMDB:
    def __init__(self, args, logger):
        self.args = args
        self.logger = logger
        self.logger.info('> using {}'.format(args.device))
        self.logger.info('> creating model {}'.format(args.model_name))

        if args.model_name == 'bert':
            self.tokenizer = AutoTokenizer.from_pretrained('/home/wyx/ConFEDE/MOSEI/bert-base-uncased')
            self.input_size = 768
            base_model = AutoModel.from_pretrained('/home/wyx/ConFEDE/MOSEI/bert-base-uncased')
        elif args.model_name == 'roberta':
            self.tokenizer = AutoTokenizer.from_pretrained('roberta-base', add_prefix_space=True)
            self.input_size = 768
            base_model = AutoModel.from_pretrained('/home/wyx/ConFEDE/MOSEI/roberta-base')
        else:
    ...
    def run(self):
        ...
        torch.save(self.Mymodel.state_dict(),self.args.method+'_model.pth')
